...
# Checking DAB GraphQL with nested query
curl -X POST \
    -H "Content-Type: application/json" \
    -d '{ "query": "query GetBooks { books { items { id title authors { items { id first_name last_name } } } } }" }' \
    http://localhost:5000/graphql | jq

curl -X POST \
    -H "Content-Type: application/json" \
    -d '{ "query": "query GetBooks {books {items {id title year pages authors { items { first_name last_name } } } } }" }' \
    http://localhost:5000/graphql | jq